...
class AuthController{
   ...
   */
	static async getConnect(resquest, response)
	{
         const auth = request.header('Authorization') || '';
         const credtional = auth.split(' ')[1];
	if (!credtional) {return response.status(401).send({ error: 'Unauthorized' })}
	const decode = Buffer.from(credtional, 'base64').toString(
        'utf-8'
	)
        const [email, password] = decode.splite(':')
